...
write(project, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', user('write the API reference'), line({ type: 'custom-title', customTitle: 'API reference' }));
write(project, 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', user('set up a docs folder'), line({ type: 'ai-title', aiTitle: 'Set up the docs folder' }));
write(project, 'cccccccc-cccc-4ccc-8ccc-cccccccccccc', user('Unit tests for the parser'));
write(project, 'dddddddd-dddd-4ddd-8ddd-dddddddddddd', user('subagent', { isSidechain: true }));
write(worktree, 'eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee', user('add a dark mode to the dashboard'), line({ type: 'ai-title', aiTitle: 'Dark mode for the dashboard' }));
// Another project that must not show up.
write(path.join(config, 'projects', 'C--some-other-project'), '99999999-9999-4999-8999-999999999999', user('another project'));
...
fs.writeFileSync(
  ...
  JSON.stringify({
    oauthAccount: { accountUuid: 'acc-1', emailAddress: 'alex@example.com', displayName: 'Alex Kim', organizationType: 'claude_max' },
...
const official = {
  ...
  [`sessionGroups:${fs.realpathSync(workspace)}`]: [
    { id: 'o1', name: 'Documentation', collapsed: false, sessionIds: ['aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', 'remote:cloud-1', 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb'] },
    { id: 'o2', name: 'Tests', collapsed: true, sessionIds: ['cccccccc-cccc-4ccc-8ccc-cccccccccccc'] },
  ],
};
